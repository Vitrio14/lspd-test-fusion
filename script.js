...
function startFinalExam() {
    ...
    document.getElementById('abort-btn').classList.remove('hidden'); // MOSTRA IL TASTO ANNULLA
...
async function abortExam() {
    ...
    localStorage.setItem("pendingNotification", "TEST ANNULLATO DALL'UTENTE");
    localStorage.setItem("pendingType", "warning");
    
    // Invia log ad Discord come "ANNULLATO"
    await sendToDiscord(userName, 0, 0, "🔴 ANNULLATO MANUALMENTE", examType);
    location.reload();
}
...
function backToIdentity() {
    // Nasconde la sezione dei corsi
    document.getElementById('course-section').classList.add('hidden');
    // Mostra di nuovo la sezione dell'inserimento nome
    document.getElementById('rp-section').classList.remove('hidden');
    showNotification("RITORNO ALL'IDENTIFICAZIONE", "warning");
}
...
async function submitCandidatura() {
    ...
    const payload = {
        ...
        embeds: [{
            ...
            footer: { text: `LSPD - Gestione reclutamenti | Registro: ${registroID}` },
            timestamp: new Date().toISOString()
        }]
    };
